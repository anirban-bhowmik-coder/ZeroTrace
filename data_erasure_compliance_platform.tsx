...
export default function App() {
  ...
  // Initial data derived from the compliance audit report
  const [devices, setDevices] = useState([
    { id: 'DEV-001', name: 'WD Blue 1TB HDD', type: 'HDD', capacity: '1TB', status: 'QUEUED', currentJob: 'ZT-043', tech: 'TECH-03' },
    { id: 'DEV-002', name: 'Samsung NVMe 512GB', type: 'NVMe / SED', capacity: '512GB', status: 'COMPLETE', currentJob: 'ZT-042', tech: 'TECH-07' },
    { id: 'DEV-003', name: 'Seagate Barracuda 2TB', type: 'HDD', capacity: '2TB', status: 'COMPLETE', currentJob: 'ZT-041', tech: 'TECH-07' },
    { id: 'DEV-004', name: 'Crucial P3 1TB Opal 2.0', type: 'SED', capacity: '1TB', status: 'COMPLETE', currentJob: 'ZT-040', tech: 'TECH-03' },
    { id: 'DEV-005', name: 'SanDisk Ultra USB', type: 'USB Flash', capacity: '64GB', status: 'FAILED_VERIFICATION', currentJob: 'ZT-039', tech: 'TECH-03' },
    { id: 'DEV-006', name: 'Hitachi Enterprise 4TB', type: 'HDD', capacity: '4TB', status: 'COMPLETE', currentJob: 'ZT-038', tech: 'TECH-07' }
  ]);

  const [jobs, setJobs] = useState([
    { jobId: 'ZT-038', deviceId: 'DEV-006', method: 'NIST 800-88 Purge - 3-pass', status: 'COMPLETE', technician: 'TECH-07', cert: 'CERT-0038-F2A0', entropy: 'High (3072 sectors)' },
    { jobId: 'ZT-039', deviceId: 'DEV-005', method: 'DoD 5220.22-M 3-pass', status: 'FAILED_VERIFICATION', technician: 'TECH-03', cert: 'WITHHELD (EVT-1008)', entropy: 'Low entropy region found' },
    { jobId: 'ZT-040', deviceId: 'DEV-004', method: 'Crypto Erase - Key Destruction', status: 'COMPLETE', technician: 'TECH-03', cert: 'CERT-0040-D1E5', entropy: 'Key Wrapping (0 risk)' },
    { jobId: 'ZT-041', deviceId: 'DEV-003', method: 'NIST 800-88 Clear', status: 'COMPLETE', technician: 'TECH-07', cert: 'CERT-0041-B7C2', entropy: 'High (4096 sectors)' },
    { jobId: 'ZT-042', deviceId: 'DEV-002', method: 'NVMe Sanitize - Crypto Erase', status: 'COMPLETE', technician: 'TECH-07', cert: 'CERT-0042-A9F3', entropy: 'Key Destruction Verified' },
    { jobId: 'ZT-043', deviceId: 'DEV-001', method: 'NIST 800-88 Purge - 3-pass', status: 'QUEUED', technician: 'TECH-03', cert: 'PENDING', entropy: 'Not Evaluated' }
  ]);
      ...
      <div className="min-h-screen bg-black text-white font-sans flex flex-col selection:bg-white selection:text-black overflow-x-hidden relative">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden opacity-25">
          <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M 100,0 Q 400,300 800,100 T 1400,400" fill="none" stroke="url(#wireGrad)" strokeWidth="1" opacity="0.5" />
            ...
            <circle cx={(networkPercentage => (networkPercentage * 14) % 1200)(networkPulse)} cy="150" r="4.5" fill="#ffffff" className="animate-pulse shadow-lg" filter="url(#glow)" />
            <circle cx={(networkPercentage => 1200 - ((networkPercentage * 18) % 1200))(networkPulse)} cy="320" r="5" fill="#ffffff" filter="url(#glow)" />
            <circle cx={(networkPercentage => (networkPercentage * 10) % 1400)(networkPulse)} cy="450" r="4" fill="#ffffff" filter="url(#glow)" />
            <circle cx={(networkPercentage => (networkPercentage * 12) % 1100)(networkPulse)} cy="220" r="3.5" fill="#cccccc" />
          </svg>
        </div>
        ...
        <header className="border-b border-zinc-800 bg-black/80 backdrop-blur-md sticky top-0 z-40 px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className="bg-white hover:bg-zinc-200 text-black font-bold px-5 py-2.5 text-sm transition-all flex items-center gap-2 transform hover:-translate-y-0.5"
            >
        ...
        <section className="relative flex-1 flex flex-col items-center justify-center px-6 py-20 lg:py-28 text-center z-10">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full justify-center max-w-md">
            <button 
              onClick={() => setView('dashboard')}
              className="w-full sm:w-auto bg-white hover:bg-zinc-200 text-black font-extrabold px-8 py-4 transition-all flex items-center justify-center gap-3 group text-base uppercase tracking-wider"
            >
          ...
          <div id="stacks" className="mt-20 max-w-4xl w-full bg-zinc-950 border border-zinc-800 p-6 sm:p-8 backdrop-blur-xl shadow-2xl relative overflow-hidden text-left">
            ...
            <div className="mt-6 pt-4 border-t border-zinc-900 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <button 
                onClick={() => setView('dashboard')}
                className="bg-white hover:bg-zinc-200 text-black font-bold px-5 py-2.5 text-xs transition-all flex items-center gap-2 uppercase tracking-wider"
              >
        ...
        <footer className="border-t border-zinc-800 bg-black py-8 text-center text-xs text-zinc-500 z-10 font-mono uppercase tracking-widest">
          ZeroTrace™ Data Erasure & Compliance Platform • © 2026 ZeroTrace Team. All Rights Reserved.
        </footer>
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans flex flex-col selection:bg-white selection:text-black">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-zinc-900 p-1.5 border border-zinc-800">
            <button 
              ...
              className={`px-4 py-2 text-sm font-medium transition-all uppercase tracking-wider ${activeTab === 'overview' ? 'bg-white text-black font-bold shadow-md' : 'text-zinc-400 hover:text-white'}`}
            >
              Dashboard Overview
            </button>
            <button 
              onClick={() => setActiveTab('jobs')}
              className={`px-4 py-2 text-sm font-medium transition-all uppercase tracking-wider ${activeTab === 'jobs' ? 'bg-white text-black font-bold shadow-md' : 'text-zinc-400 hover:text-white'}`}
            >
              Jobs & Verification
            </button>
            <button 
              onClick={() => setActiveTab('devices')}
              className={`px-4 py-2 text-sm font-medium transition-all uppercase tracking-wider ${activeTab === 'devices' ? 'bg-white text-black font-bold shadow-md' : 'text-zinc-400 hover:text-white'}`}
            >
              Device Inventory
            </button>
          </div>
          ...
          <button 
            ...
            className="bg-zinc-900 hover:bg-zinc-800 text-zinc-300 text-xs font-semibold px-3.5 py-2.5 border border-zinc-700 transition-colors uppercase tracking-wider"
          >
            Back to Home
          </button>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-zinc-950 border border-zinc-800 p-5 flex items-center justify-between shadow-xl">
                <div>
                  ...
                  <h3 className="text-3xl font-black text-white mt-1">6 Units</h3>
                  <span className="text-xs font-mono text-zinc-400 mt-2 inline-block">3 HDD, 3 NVMe/SED/USB</span>
                </div>
              ...
              <div className="bg-zinc-950 border border-zinc-800 p-5 flex items-center justify-between shadow-xl">
                <div>
                  ...
                  <h3 className="text-3xl font-black text-white mt-1">4 Jobs</h3>
                  <span className="text-xs font-mono text-zinc-300 mt-2 inline-block">NIST 800-88 & Crypto Erase</span>
                </div>
                <div className="p-3 bg-zinc-900 border border-zinc-700 text-white">
              ...
              <div className="bg-zinc-950 border border-zinc-800 p-5 flex items-center justify-between shadow-xl">
                <div>
                  ...
                  <h3 className="text-3xl font-black text-zinc-300 mt-1">1 Incident</h3>
                  <span className="text-xs font-mono text-zinc-400 mt-2 inline-block">DEV-005 Low Entropy</span>
                </div>
              ...
              <div className="bg-zinc-950 border border-zinc-800 p-5 flex items-center justify-between shadow-xl">
                <div>
                  ...
                  <h3 className="text-3xl font-black text-zinc-400 mt-1">1 Queued</h3>
                  <span className="text-xs font-mono text-zinc-400 mt-2 inline-block">Job ZT-043 (DEV-001)</span>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 bg-zinc-950 border border-zinc-800 p-6 shadow-xl flex flex-col justify-between">
                ...
                <div className="mt-6 flex gap-4">
                  <button 
                    onClick={() => setActiveTab('jobs')}
                    className="flex-1 bg-white hover:bg-zinc-200 text-black font-bold py-3 px-4 shadow-lg transition-all flex items-center justify-center gap-2 uppercase tracking-wider text-xs"
                  >
                  ...
                  <div className="space-y-4">
                    <div className="p-3.5 bg-black border border-zinc-700">
                      ...
                      <button 
                        ...
                        className="mt-3 w-full bg-white hover:bg-zinc-200 text-black text-xs font-bold py-2 transition-colors uppercase tracking-wider"
                      >
                    ...
                    <div className="p-3.5 bg-black border border-zinc-800">
                      ...
                      <button 
                        onClick={() => handleDispatchJob('ZT-043')}
                        className="mt-3 w-full bg-zinc-900 hover:bg-zinc-800 text-white font-bold text-xs py-2 border border-zinc-700 transition-colors uppercase tracking-wider"
                      >
                        Dispatch Job ZT-043
                      </button>
                    </div>
              ...
              <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto">
                {['ALL', 'COMPLETE', 'FAILED_VERIFICATION', 'QUEUED'].map((status) => (
                  <button
                    ...
                    className={`px-3 py-2 text-xs font-mono uppercase tracking-wider whitespace-nowrap transition-all border ${filterStatus === status ? 'bg-white text-black font-bold border-white' : 'bg-black text-zinc-300 border-zinc-800 hover:border-zinc-600'}`}
                  >
                    {status.replace('_', ' ')}
                  </button>
                ))}
                    ...
                    {filteredJobs.map((job) => (
                      <tr key={job.jobId} className="hover:bg-zinc-900/40 transition-colors">
                        ...
                        <td className="p-4 text-right">
                          {job.status === 'QUEUED' ? (
                            <button 
                              onClick={() => handleDispatchJob(job.jobId)}
                              className="bg-white hover:bg-zinc-200 text-black font-bold px-3 py-1.5 text-xs transition-colors flex items-center gap-1 ml-auto uppercase tracking-wider"
                            >
                              <Play className="w-3 h-3" /> Run Wipe
                            </button>
                          ) : job.status === 'COMPLETE' ? (
                            <button 
                              onClick={() => setSelectedCert(job)}
                              className="bg-zinc-900 hover:bg-zinc-800 text-white px-3 py-1.5 text-xs font-semibold border border-zinc-700 transition-colors flex items-center gap-1 ml-auto uppercase tracking-wider"
                            >
                  ...
                  <div className="pt-4 border-t border-zinc-900 flex items-center justify-between">
                    ...
                    {dev.status === 'FAILED_VERIFICATION' ? (
                      <button 
                        ...
                        className="bg-white hover:bg-zinc-200 text-black text-xs font-bold px-3 py-1.5 transition-colors uppercase tracking-wider"
                      >
                        Quarantine
                      </button>
                    ) : (
          ...
          <div className="bg-zinc-950 border border-zinc-700 max-w-lg w-full p-6 shadow-2xl relative">
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button 
                ...
                className="bg-white hover:bg-zinc-200 text-black font-bold px-4 py-2 text-xs transition-colors uppercase tracking-wider"
              >
                Download PDF Certificate
              </button>
              <button 
                onClick={() => setSelectedCert(null)}
                className="bg-zinc-900 hover:bg-zinc-800 text-zinc-300 px-4 py-2 text-xs font-semibold border border-zinc-700 transition-colors uppercase tracking-wider"
              >
                Close
              </button>
            </div>
      ...
      <footer className="border-t border-zinc-800 bg-black py-6 text-center text-xs text-zinc-500 font-mono uppercase tracking-widest">
        ZeroTrace™ Data Erasure & Compliance Dashboard • Built with React & Tailwind CSS • © 2026 ZeroTrace Team. All Rights Reserved.
      </footer>
